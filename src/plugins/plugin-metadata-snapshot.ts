...
/** Clear process-local plugin metadata snapshot memoization after install/reload lifecycle changes. */
...
/** Check whether a cached/current metadata snapshot still matches config, env, and index state. */
...
/** Build a plugin-id to origin lookup from an already-resolved metadata snapshot. */
...
/** Load and freeze a plugin metadata snapshot, using bounded process memoization when safe. */
...
/** Prefer the current process snapshot when compatible, otherwise load a fresh snapshot. */
