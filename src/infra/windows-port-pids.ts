...
/** Result from Windows listener-PID discovery with retryability metadata. */
export type WindowsListeningPidsResult =
  | { ok: true; pids: number[] }
  | { ok: false; permanent: boolean };

/** Result from Windows process command-line discovery with retryability metadata. */
...
/** Returns listener PIDs for a Windows TCP port, falling back to an empty list on failure. */
...
/** Returns Windows TCP listener PIDs plus whether failure is permanent. */
...
/** Returns parsed argv for a Windows process, or null on failure. */
...
/** Returns parsed Windows process argv plus whether failure is permanent. */
