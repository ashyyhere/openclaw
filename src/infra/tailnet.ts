...
/** Return whether an IPv4 address belongs to the Tailscale carrier-grade NAT range. */
...
/** List unique local interface addresses that belong to known Tailscale ranges. */
...
/** Pick the first discovered Tailnet IPv4 address, if present. */
export function pickPrimaryTailnetIPv4(): string | undefined {
  return listTailnetAddresses().ipv4[0];
}

/** Pick the first discovered Tailnet IPv6 address, if present. */
