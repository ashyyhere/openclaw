...
/** Delay before the first work event starts a progress draft automatically. */
...
/** Return whether a tool name should count as visible work in progress drafts. */
...
/** Detect final text that likely ended with a transport-imposed ellipsis truncation. */
...
/** Prefer a transcript candidate when it extends an ellipsis-truncated final message. */
...
/** Recover a fuller final message from transcript text when the channel final is truncated. */
...
/** Formatting options for converting stream events into progress draft lines. */
...
/** Progress draft render target requested by channel config. */
export type ChannelProgressDraftRenderMode = "text" | "rich";

const EMOJI_PREFIX_RE = /^\p{Extended_Pictographic}/u;

/** Normalized stream event input accepted by progress-line builders. */
...
/** Event family represented by a progress draft line. */
export type ChannelProgressDraftLineKind = ChannelProgressDraftLineInput["event"];

/** Render-ready progress draft line with stable identity and display metadata. */
...
/** Format a progress event as a single display line, dropping events with no useful text. */
...
/** Merge explicit formatting options with channel-level command text mode. */
...
/** Build a structured progress line using channel config fallback options. */
...
/** Format a progress event using channel config fallback options. */
...
/** Convert a normalized stream event into structured progress draft metadata. */
...
/** Gate progress draft startup until work repeats or the initial delay elapses. */
...
/** Read the structured `streaming` config object when present. */
...
/** Resolve text chunk mode from modern streaming config or legacy top-level fields. */
...
/** Resolve whether block streaming is enabled from modern or legacy config fields. */
...
/** Resolve block coalescing config from modern or legacy config fields. */
...
/** Resolve preview chunk config from modern or legacy config fields. */
...
/** Resolve whether preview streaming should include tool progress lines. */
...
/** Resolve whether progress-mode streaming should send commentary updates. */
...
/** Resolve whether command progress should show raw command text or status text. */
...
/** Resolve whether native transport tool progress should appear in preview streams. */
...
/** Resolve allow-list metadata for native transport tool progress events. */
...
/** Decide if default tool-progress messages should be suppressed by active preview streaming. */
...
/** Resolve whether the channel should use native transport streaming. */
...
/** Resolve preview stream mode from modern config, legacy fields, then default. */
...
/** Read progress-draft config, returning an empty object for missing config. */
...
/** Resolve a progress draft label from explicit config, seeded choice, or random choice. */
...
/** Resolve the maximum retained progress draft lines. */
...
/** Resolve the maximum visible characters per progress draft line. */
...
/** Resolve whether progress draft output should be text or rich metadata. */
...
/** Normalize a line into the stable identity used for duplicate suppression. */
...
/** Merge a progress line into the retained window, updating by id and dropping duplicates. */
...
/** Render the bounded progress draft text block with optional label and bullets. */
